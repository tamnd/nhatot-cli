...
describes what users get. Pick a format with `-o`, or let nhatot choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
nhatot <command> -o table   # aligned columns for reading
nhatot <command> -o jsonl   # one JSON object per line, for piping
nhatot <command> -o json    # a single JSON array
nhatot <command> -o csv     # spreadsheet friendly
nhatot <command> -o tsv     # tab-separated
nhatot <command> -o url     # just the URL column
nhatot <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
nhatot <command> --fields id,title,url
```
...
```bash
nhatot <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
nhatot <command>            # a table, because this is a terminal
nhatot <command> | wc -l    # JSONL, because this is a pipe
```
